...
# 페이지 설정
st.set_page_config(page_title="제이제이컴퍼니 스마트 포털 V6.0", layout="wide", initial_sidebar_state="expanded")

# [V6.0] 1단계: 철통 보안 로그인 (Login) 시스템
...
if not st.session_state.logged_in:
    ...
    with col2:
        st.markdown("""
        <div style='background: rgba(255, 255, 255, 0.05); padding: 40px; border-radius: 20px; text-align: center; border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(10px); box-shadow: 0 4px 30px rgba(0, 0, 0, 0.5);'>
            <h1 style='color: #00e5ff; margin-bottom: 10px;'>🚀 JJ COMPANY</h1>
            <h3 style='color: #e0e6ed; margin-bottom: 30px;'>관제탑 보안 게이트웨이</h3>
            <p style='color: #8892b0; margin-bottom: 20px;'>인가된 관리자만 접근할 수 있는 프라이빗 시스템입니다.</p>
        </div>
        """, unsafe_allow_html=True)
        
        pwd = st.text_input("🔑 관리자 패스워드를 입력하세요", type="password", key="admin_pwd_input", label_visibility="collapsed", placeholder="초기 패스워드: jjcompany123")
...
if st.session_state.app_mode == "📂 프라이빗 웹/파일 뷰어":
    ...
    st.info("💡 **사용 가이드**: 아래 거대한 텍스트 박스 공간에 준비하신 문서를 마음껏 붙여넣으세요. 코드가 섞여 있다면 `🚀 HTML 렌더링 실행` 버튼을 눌러 디자인 그래픽을 실시간으로 확인하실 수 있습니다.")
    
    viewer_tab1, viewer_tab2 = st.tabs(["HTML 그래픽 렌더링 뷰", "단순 텍스트 편집기 뷰"])
    
    with viewer_tab1:
        html_input = st.text_area("HTML 코드 입력 (복사 후 붙여넣기)", height=500, placeholder="<html><body><h1>Hello JJ Company!</h1>...</body></html>", key="html_input_box")
        if st.button("🚀 HTML 렌더링 뷰 즉시 가동", use_container_width=True):
            if html_input.strip():
                st.markdown("### 🖥️ 디자인(DOM) 렌더링 결과")
                st.markdown("---")
                components.html(html_input, height=800, scrolling=True)
            else:
                st.warning("⚠️ 렌더링할 소스 코드를 제일 먼저 텍스트 박스에 넣어 주세요!")
                
    with viewer_tab2:
        plain_input = st.text_area("텍스트 / 마크다운 문서 자유 입력 공간", height=700, placeholder="자유롭게 붙여넣기 하십시오...", key="plain_input_box")
            
    # 웹뷰어에서 로직이 끝나면 하단 1400줄의 구형 B2B 관제탑 코드가 메모리에 뜨지 않도록 또 한 번 스탑!
    st.stop()

# =========================================================================
# 만약 app_mode가 '🏢 B2B 메인 관제탑' 이라면 위의 st.stop()을 회피하여
# 아래 기존 1400줄 관제탑(V5) 아키텍처 코드가 순둥이처럼 100% 온전하게 이어서 실행됩니다!
# =========================================================================
